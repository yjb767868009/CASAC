...
parser.add_argument("--base_train", help="train key prediction model", action="store_true")
...
if __name__ == "__main__":
    train_flag = args.key_train or args.motion_train or args.base_train
    model = initialization(epoch=args.epoch, batch_size=args.batch_size, data_root=args.data_root,
                           ...
                           train=train_flag)
    if train_flag:
        model.train(args.pretrain, args.base_train, args.key_train, args.motion_train)
    if args.test:
